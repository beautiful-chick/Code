 ...
 *      ChangeLog:  1, Release initial version on "20/03/24 20:28:34"
 *                 
 ********************************************************************************/
#include <unistd.h>
#include <string.h>
	...
	for( ; ; )
	{
		...
		else
		{
			for(i=0; i<ARRAY_SIZE(fds_array); i++)
			{
				...
				{
					printf("soxket[%d] read failure or get disconnect.\n", fds_array[i]);
					close(fds_array[i]);
					fds_array[i] = -1;
	   			}
		  		else
...
static inline void print_usage(char *progname)
{
	...
	printf(" %s is a socket server program, which usedverify client and echo back string from it\n",progname); 
...
int socket_server_init(char *listen_ip, int listen_port)
{
	...
	setsockopt(listenfd, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on));//重用地址
	...
	{
		printf("Use bind() to bind the TCP server failure: %s\n", strerror(errno));
		rv = -4;
		goto CleanUp;
	}
CleanUp:
