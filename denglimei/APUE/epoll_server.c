 ...
 *      ChangeLog:  1, Release initial version on "25/03/24 21:04:18"
 *                 
 ********************************************************************************/
...
int main(int argc, char **argv)
{
	...
	for( ; ;)
	{
		...
		for(i=0; i<events; i++) //遍历就绪的文件描述符的数量
		{
			...
			else
			{
				...
				else
				{
					...
					{
						printf("socket[%d] write faiure: %s\n", event_array[i].data.fd, strerror(errno));
						epoll_ctl(epollfd, EPOLL_CTL_DEL, event_array[i].data.fd, NULL);
						close(event_array[i].data.fd);
					}
				}
...
static inline void print_usage(char *progname)
{
	...
	printf(" -p [port] set program running on background\n");
	printf(" -h [help] socket server port address\n");
...
int socket_server_init(char *listen_ip, int listen_port)
{
	...
	{
		printf("Use socket() to  create a TCP server failure: %s\n", strerror(errno));
		return -1;
	}
